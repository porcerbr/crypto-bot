# -*- coding: utf-8 -*-
"""
BOT SNIPER v7.1 — Multi-mercado + Confirmação Manual + Cópia Individual SL/TP
═══════════════════════════════════════════════════════════════════════════════
ALTERAÇÕES APLICADAS (conforme solicitado):
✅ Gatilho + Confluência → Cria Operação Pendente (não abre trade automático)
✅ Notificação enviada aguardando resposta (Aceitar/Recusar)
✅ Aceitar → Trade Aberto | Recusar → Descartado
✅ Cópia individual de SL e TP (apenas valores numéricos)
...
class Config:
    ...
    TIMEFRAMES = {
        "1m": ("Agressivo", "7d"), "5m": ("Alto", "5d"), "15m": ("Moderado", "5d"),        "30m": ("Conservador", "5d"), "1h": ("Seguro", "60d"), "4h": ("Muito Seguro", "60d")
    }
    TIMEFRAME = "15m"
...
def load_state(bot):
    ...
    try:
        ...
        log(f"[STATE] {bot.wins}W/{bot.losses}L | {len(bot.active_trades)} trade(s) | {len([o for o in bot.pending_operations if o['status']=='PENDING'])} pendente(s)")
...
def get_analysis(symbol, timeframe=None):
    ...
    try:
        ...
        ema200 = closes.ewm(span=min(200, len(closes)-1), adjust=False).mean().iloc[-1]        w = min(20, len(closes)-1)
...
def __init__(self):
    ...
    def update_trends_cache(self):
        ...
        for s in all_syms():
            try:                res = get_analysis(s, self.timeframe)
                if res:
    ...
    # ── NOVO: GESTÃO DE OPERAÇÕES PENDENTES ──────────────────
    def create_pending_operation(self, symbol, direction, price, sl, tp, res, sc, tot_c, bar, conf_txt):
        op_id = f"{symbol}_{int(time.time()*1000)}"
        op = {
            "id": op_id, "symbol": symbol, "name": res["name"], "direction": direction,
            "entry": price, "sl": sl, "tp": tp, "atr": res["atr"], "rsi": res["rsi"], "adx": res["adx"],
            "created_at": datetime.now(Config.BR_TZ).strftime("%d/%m %H:%M:%S"), "status": "PENDING",
            "confluence_score": sc, "confluence_total": tot_c, "confluence_bar": bar, "confluence_txt": conf_txt
        }
    ...
    def _send_pending_alert(self, op):
        ...
        msg = (f"⏳ <b>OPERAÇÃO PENDENTE – {op['symbol']}</b> ({op['name']})\n"
               f"TF: <code>{self.timeframe}</code>\n\n"
               f"╔══════════════════╗\n  ▶️   <b>{dl}</b>\n╚══════════════════╝\n\n"
               f"💰 <b>Entrada:</b> <code>{fmt(op['entry'])}</code>\n"
               f"🛡 <b>SL:</b> <code>{fmt(op['sl'])}</code>\n"
               f"🎯 <b>TP:</b> <code>{fmt(op['tp'])}</code>\n\n"
               f"<i>Aceite ou recuse no app/dashboard.</i>")
        self.send(msg)
        threading.Thread(target=send_push, args=("⏳ Operação Pendente", f"{op['symbol']} {op['direction']} | Entrada: {fmt(op['entry'])}"), daemon=True).start()
    ...
    def confirm_pending_operation(self, op_id):
        ...
        trade = {"symbol": op["symbol"], "name": op["name"], "entry": op["entry"], "tp": op["tp"], "sl": op["sl"],
                 "dir": op["direction"], "peak": op["entry"], "atr": op["atr"], "opened_at": op["created_at"], "session_alerted": True}
    ...
    def ignore_pending_operation(self, op_id):
        op = next((o for o in self.pending_operations if o["id"] == op_id), None)
        if not op: return False        op["status"] = "IGNORED"
        log(f"[IGNORE] {op['symbol']} {op['direction']} rejeitada")
        save_state(self)
        return True

    # ── SCAN MODIFICADO: FASE 4B AGORA CRIA PENDENTE ─────────
    def scan(self):
        ...
        for s in universe:
            ...
            # FASE 4B — CRIA PENDENTE (AGUARDA CONFIRMAÇÃO MANUAL)
            ...
            self.create_pending_operation(s, dir_s, price, sl_final, tp_final, res, sc, tot_c, bar, conf_txt)
    ...
    def scan_reversal_forex(self):
        ...
        for s in Config.MARKET_CATEGORIES["FOREX"]["assets"].keys():
            ...
            sl_m = Config.ATR_MULT_SL; tp_m = Config.ATR_MULT_SL * 1.5            sl = price - sl_m*atr if dir_s == "BUY" else price + sl_m*atr
            ...
            self.send(f"⚡ <b>CONTRA-TENDÊNCIA FOREX – {s}</b> ({res['name']})\n📈 FOREX | TF: <code>{self.timeframe}</code>\n\n"
                      f"🔄 <i>Sinal de reversão — CONTRA a tendência</i>\n\n╔══════════════════╗\n  ▶️   <b>{dl}</b>\n╚══════════════════╝\n\n"
                      f"💰 <b>Entrada:</b> <code>{fmt(price)}</code>\n🛡 <b>SL:</b> <code>{fmt(sl)}</code> ({-sl_p:.2f}%)\n"
                      f"🎯 <b>TP:</b> <code>{fmt(tp)}</code> ({tp_p:+.2f}%)\n⚖️ <b>Ratio:</b> <b>{ratio}</b>\n\n"
                      f"<b>Sinais de exaustão:</b>\n{sinais_txt}\n\nADX: <code>{res['adx']:.1f}</code> | RSI: <code>{res['rsi']:.1f}</code>\n\n"
                      f"<b>Confluência: {sc}/{tc} [{bar}]</b>\n{conf_txt}\n\n⚠️ <i>Use gestão de risco reduzida.</i>")
            self.reversal_list[s] = time.time()
            # CT também vai para pendente
            self.create_pending_operation(s, dir_s, price, sl, tp, res, sc, tc, bar, conf_txt)
...
.ldot{width:6px;height:6px;border-radius:50%;background:var(--green);animation:blink 2s ease-in-out infinite}@keyframes blink{0%,100%{opacity:1}50%{opacity:.3}}
...
.pg::-webkit-scrollbar{width:2px}.pg::-webkit-scrollbar-thumb{background:var(--border);border-radius:2px}
...
.nb .ni{font-size:18px;transition:.2s}.nb.on{color:var(--green)}.nb.on .ni{transform:scale(1.1)}.nb:active{opacity:.7}
.nbadge{position:absolute;top:6px;right:calc(50% - 17px);min-width:15px;height:15px;border-radius:8px;background:var(--red);color:#fff;font-size:8px;display:none;align-items:center;justify-content:center;font-family:var(--mono);font-weight:700;padding:0 4px}
...
.mktn{font-size:11px;font-weight:500}.mkts{font-size:8px;letter-spacing:.8px;text-transform:uppercase;padding:2px 8px;border-radius:20px;font-family:var(--mono)}
...
.pgrid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.pbox{background:var(--bg3);border:1px solid var(--border);border-radius:var(--rsm);padding:12px}
...
.empi{font-size:40px;margin-bottom:10px;display:block}.empt{font-size:12px;line-height:1.6;color:var(--muted2)}
...
.cfgsec{margin-bottom:18px}.cfgl{font-size:9px;letter-spacing:1.5px;text-transform:uppercase;color:var(--muted2);margin-bottom:10px}
...
.pc-btn{flex:1;min-width:70px;padding:8px 10px;border:none;border-radius:var(--rsm);font-size:11px;font-weight:600;cursor:pointer;font-family:var(--sans);transition:.15s;letter-spacing:.3px}
...
<div id="app">
  <div id="hdr">
    <div class="hdr-l"><div class="logo">S</div><div><div class="t1">Sniper Bot</div><div class="t2">Multi-Mercado v7.1</div></div></div>
    <div class="hdr-r"><div class="lpill"><div class="ldot"></div>LIVE</div><div class="ibtn" id="refbtn" onclick="refreshAll()">↻</div></div>  </div>
  <div id="pages">
    ...
    <div id="pg-scan" class="pg">
      ...
      <div class="fchips" id="scan-filters">
        <button class="fc on" data-cat="TODOS" onclick="setSF('TODOS',this)">Todos</button>
        <button class="fc" data-cat="ALTA" onclick="setSF('ALTA',this)">🟢 Alta</button>
        <button class="fc" data-cat="BAIXA" onclick="setSF('BAIXA',this)">🔴 Baixa</button>
        <button class="fc" data-cat="FOREX" onclick="setSF('FOREX',this)">📈 Forex</button>
        <button class="fc" data-cat="CRYPTO" onclick="setSF('CRYPTO',this)">₿ Cripto</button>
        <button class="fc" data-cat="COMMODITIES" onclick="setSF('COMMODITIES',this)">🏅 Comm.</button>
        <button class="fc" data-cat="INDICES" onclick="setSF('INDICES',this)">📊 Índices</button>
      </div>
    ...
    <div id="pg-sig" class="pg">
      ...
      <div class="fchips" id="sig-filters">
        <button class="fc on" data-sf="todos" onclick="setSigF('todos',this)">Todos</button>
        <button class="fc" data-sf="sinal" onclick="setSigF('sinal',this)">🎯 Sinal</button>
        <button class="fc" data-sf="gatilho" onclick="setSigF('gatilho',this)">🔔 Gatilho</button>
        <button class="fc" data-sf="radar" onclick="setSigF('radar',this)">⚠ Radar</button>
        <button class="fc" data-sf="ct" onclick="setSigF('ct',this)">⚡ CT</button>
        <button class="fc" data-sf="close" onclick="setSigF('close',this)">🏁 Fechados</button>
      </div>
    ...
    <div id="pg-ct" class="pg">
      ...
      <div class="sh"><span class="sttl">⚡ Oportunidades CT (FOREX)</span><button class="ibtn" onclick="loadCT()">↻</button></div>
      <div id="ct-list"><div class="empty"><span class="empi">⚡</span><div class="empt">Nenhuma oportunidade CT detectada.</div></div></div>      <div class="dv"></div>
      <div class="sh"><span class="sttl">📰 Notícias</span><button class="ibtn" onclick="loadNews()">↻</button></div>
...
<script>
let _st=null,_scan=[],_sigs=[],_pend=[],_sf='TODOS',_sigf='todos',_unread=0,_lastSigLen=0;
...
async function refreshAll(){const b=document.getElementById('refbtn');b.classList.add('spin');try{await loadDash();const a=document.querySelector('.pg.on');if(a.id==='pg-scan')await loadScanner();if(a.id==='pg-sig')await loadSigs();if(a.id==='pg-pend')await loadPending();}finally{b.classList.remove('spin');}}
...
async function loadScanner(){try{_scan=await apiFetch('/api/trends');let data=[..._scan];if(_sf==='ALTA')data=data.filter(x=>x.cenario==='ALTA');else if(_sf==='BAIXA')data=data.filter(x=>x.cenario==='BAIXA');else if(['FOREX','CRYPTO','COMMODITIES','INDICES'].includes(_sf))data=data.filter(x=>x.category===_sf);data.sort((a,b)=>({ALTA:0,BAIXA:1,NEUTRO:2}[a.cenario]??2)-({ALTA:0,BAIXA:1,NEUTRO:2}[b.cenario]??2));if(!data.length){document.getElementById('scan-list').innerHTML='<div class="empty"><span class="empi">🔍</span><div class="empt">Nenhum ativo neste filtro.</div></div>';return;}document.getElementById('scan-list').innerHTML=data.map(x=>{const t=x.cenario||'NEUTRO',ic=t==='ALTA'?'↑':t==='BAIXA'?'↓':'–',icc=t==='ALTA'?'siA':t==='BAIXA'?'siB':'siN';const rc=x.rsi>70?'tr':x.rsi<30?'tg':'tn',ac=x.adx>25?'tg':'tn';const chgC=x.change_pct>0?'g':x.change_pct<0?'r':'';return`<div class="si"><div class="sico ${icc}">${ic}</div><div class="sinf"><div class="ssym">${x.symbol}</div><div class="snm">${x.name||''} · ${x.category||''}</div></div><div class="srgt"><div class="spr">${fp(x.price)}</div><div class="stags"><span class="tag ${chgC?'t'+chgC:'tn'}">${x.change_pct>=0?'+':''}${x.change_pct.toFixed(2)}%</span> <span class="tag ${rc}">RSI ${x.rsi.toFixed(0)}</span> <span class="tag ${ac}">ADX ${x.adx.toFixed(0)}</span></div></div></div>`;}).join('');document.getElementById('scan-ts').textContent='Atualizado '+new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});}catch(e){document.getElementById('scan-list').innerHTML='<div class="empty"><span class="empi">⚠</span><div class="empt">Erro ao carregar</div></div>';}}
function setSF(cat,el){document.querySelectorAll('#scan-filters .fc').forEach(c=>c.classList.remove('on'));el.classList.add('on');_sf=cat;loadScanner();}
async function loadSigs(){try{const d=await apiFetch('/api/signals');if(d.length>_lastSigLen){_unread+=d.length-_lastSigLen;updBadge();}_lastSigLen=d.length;_sigs=d;renderSigs();document.getElementById('sig-ts').textContent='Atualizado '+new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});}catch(e){}}
function setSigF(f,el){document.querySelectorAll('#sig-filters .fc').forEach(x=>x.classList.remove('on'));el.classList.add('on');_sigf=f;renderSigs();}
...
function renderSigs(){let d=[..._sigs];if(_sigf!=='todos')d=d.filter(x=>x.tipo===_sigf);const el=document.getElementById('sig-list');if(!d.length){el.innerHTML='<div class="empty"><span class="empi">🔔</span><div class="empt">Nenhum sinal neste filtro.</div></div>';return;}el.innerHTML=d.map(s=>{const m=SM[s.tipo]||SM.radar;return`<div class="sigit"><div class="sgico ${m.cls}">${m.icon}</div><div class="sgbody"><div class="sgtipo" style="color:${m.c}">${m.lbl}</div><div class="sgtxt">${s.texto}</div><div class="sgts">${s.ts}</div></div></div>`;}).join('');}
function updBadge(){const b=document.getElementById('nbadge-sig');if(_unread>0){b.style.display='flex';b.textContent=_unread>9?'9+':_unread;}else b.style.display='none';}
async function loadCT(){const el=document.getElementById('ct-list');el.innerHTML='<div class="empty"><span class="empi spin">⚡</span><div class="empt">Analisando…</div></div>';try{const d=await apiFetch('/api/reversals');if(!d.length){el.innerHTML='<div class="empty"><span class="empi">⚡</span><div class="empt">Nenhuma oportunidade CT detectada.</div></div>';return;}el.innerHTML=d.map(x=>{const buy=x.direction==='BUY';return`<div class="ctc"><div class="cttop"><span class="ctsym">${x.symbol} <span style="font-size:10px;color:var(--muted2)">${x.name}</span></span> <span class="ctsc">${x.strength}%</span></div><div class="ctdir ${buy?'g':'r'}">${buy?'▲ COMPRAR':'▼ VENDER'} — ${buy?'Baixa→Alta':'Alta→Baixa'}</div><div class="ctsigs">${(x.reasons||[]).map(s=>`<span class="ctag">${s}</span>`).join('')}</div><div class="ctm2"><div class="ctmb"><div class="ctml">Preço</div><div class="ctmv">${fp(x.price)}</div></div><div class="ctmb"><div class="ctml">RSI</div><div class="ctmv ${x.rsi>70?'r':x.rsi<30?'g':''}">${x.rsi.toFixed(1)}</div></div><div class="ctmb"><div class="ctml">Força</div><div class="ctmv go">${x.strength}%</div></div></div></div>`;}).join('');}catch(e){el.innerHTML='<div class="empty"><span class="empi">⚠</span><div class="empt">Erro ao carregar</div></div>';}}
async function loadNews(){const el=document.getElementById('news-list');try{const d=await apiFetch('/api/news');const fg=d.fg,arts=d.articles||[];if(fg){document.getElementById('fgv').textContent=fg.value+' – '+fg.label;const n=parseInt(fg.value)||0;document.getElementById('fgn').textContent=n;document.getElementById('fgv').style.color=n>60?'var(--green)':n<40?'var(--red)':'var(--gold)';}if(!arts.length){el.innerHTML='<div class="empty"><span class="empi">📰</span><div class="empt">Sem notícias.</div></div>';return;}el.innerHTML=arts.map((a,i)=>`<div class="ni2"><div class="nnum">${i+1}</div><div class="nbody"><a class="ntitle" href="${a.url||'#'}" target="_blank">${a.title||''}</a><div class="nsrc">${a.source||''}</div></div></div>`).join('');}catch(e){el.innerHTML='<div class="empty"><span class="empi">⚠</span><div class="empt">Erro ao carregar</div></div>';}}
...
function updBadges(){const pend=_pend?_pend.length:0;document.getElementById('nbadge-pend').textContent=pend>0?pend:'';document.getElementById('nbadge-pend').style.display=pend>0?'flex':'none';}
window.addEventListener('load',()=>{loadDash();loadPending();setInterval(()=>{loadDash();if(document.querySelector('.pg.on').id==='pg-pend')loadPending();if(document.querySelector('.pg.on').id==='pg-scan')loadScanner();if(document.querySelector('.pg.on').id==='pg-sig')loadSigs();},10000);if('serviceWorker' in navigator)navigator.serviceWorker.register('/sw.js').catch(()=>{});});
</script>
</body>
</html>"""
...
def create_api(bot):
    ...
    def api_pending():
        return jsonify([o for o in bot.pending_operations if o["status"] == "PENDING"])
    ...
    @app.route("/api/news")    def api_news():
        now = time.time()
    ...
    def api_reset():
        if request.method == "OPTIONS": return jsonify({}), 200        bot.reset_pause(); return jsonify({"ok": True})
            ...
            if "result" in r:
                for u in r["result"]:
                    ...
                    if "callback_query" in u:
                        ...
                        elif cb == "main_menu": bot.build_menu()                        elif cb == "news": bot.send_news()
...
def main():
    log("🔌 Bot Sniper v7.1 — Confirmação Manual + Cópia Individual SL/TP")
