                ...
                for u in r["result"]:
                    ...
                    if "message" in u:
                        ...
                        if bot.awaiting_custom_amount and txt not in ("/cancelar", "cancelar"):
                            ...
                            except: bot.send("❌ Envie apenas um valor numérico válido, por exemplo: <code>500</code>")
                            ...
                            try:
                                ...
                                if bot.set_balance(val): bot.send(f"✅ Saldo ajustado para <code>{fmt(bot.balance)}</code>")
                                else: bot.send("❌ Saldo inválido. Use: <code>/setsaldo 500</code>")
                            except: bot.send("❌ Use: <code>/setsaldo 500</code>")
                    ...
                    if "callback_query" in u:
                        ...
                        if cb.startswith("set_tf"): bot.set_timeframe(cb.replace("set_tf_", ""))
                            ...
                            try:
                                parte = cb.split("_", 2)
                                pid = int(parte[2]); amt = float(parte[1])
