# -*- coding: utf-8 -*-
"""
TICKMILL SNIPER BOT v9.0 — Dashboard Profissional MT5
══════════════════════════════════════════════════════════════════════════
CORRETORA: Tickmill | Plataforma: MT5 | Conta: Raw ECN (USD)

MUDANÇAS v9.0:
✅ Cálculo de margem 100% alinhado com fórmula oficial Tickmill
✅ SL/TP em % automático baseado na alavancagem (ratio fixo 1:2.5)
✅ Investimento em USD direto — bot calcula o lote automaticamente
✅ Comissão Raw ECN corrigida ($3/side por lote padrão)
✅ Contract sizes MT5 oficiais Tickmill
✅ Dashboard simplificada com botões de valor rápido
✅ Todos os cálculos de risco revisados e corrigidos
"""
...
class Config:
    ...
    # ── Alavancagem & Risco ─────────────────────────────────────
    # SL/TP automático: quanto maior a alavancagem, menor a %
    # Fórmula: sl_pct = min(3.0, max(0.2, 250 / leverage))
    # tp_pct = sl_pct * 2.5  (ratio fixo 1:2.5)
    ...
    # Forex e Metais: $3/side = $6 RT por lote padrão
    # Na prática: commission = $3 × lot × 2 (open+close)
...
def calc_margin(symbol, price, leverage, lot):
    """
    Margem = Lot × ContractSize × Price / Leverage
    Ajustado para pares onde USD é base (não precisa do preço para margem em USD)
    """
...
def calc_tick_value_usd(symbol, price=None):
    """
    Retorna o valor monetário de 1 pip/tick por lote PADRÃO (1.00) em USD.
    """
    profile = symbol_profile(symbol)
    cs = float(profile["contract_size"])
    kind = profile["kind"]
    ...
    if kind == "FX":
        # 1 pip = 0.0001 (ou 0.01 para JPY)
        pip_size = 0.01 if str(symbol).upper().endswith("JPY") else 0.0001
        if quote == "USD":
            # EURUSD: 0.0001 × 100000 = $10 por lote padrão
            return pip_size * cs
        elif base == "USD":
            # USDJPY: 0.01 × 100000 = 1000 JPY por lote → converter para USD
            usd_jpy = currency_to_usd("JPY") if price is None else 1.0/price
            return pip_size * cs * usd_jpy
        else:
            # Cross: converter para USD via moeda de cotação
            quote_to_usd = currency_to_usd(quote)
            return pip_size * cs * quote_to_usd
    elif symbol == "XAUUSD":
        # Ouro: 1 pip = $0.01, contract = 100 oz → $1 por pip por lote
        return 1.0
    elif symbol == "XAGUSD":
        # Prata: 1 pip = $0.01, contract = 5000 oz → $50 por pip por lote  
        return 50.0
    elif symbol in ("XTIUSD", "BRENT"):
        # Oil: 1 pip = $0.01, contract = 1000 barris → $10 por pip por lote
        return 10.0
    elif symbol in ("NATGAS", "COPPER"):
        # 1 pip = $0.01, contract = 1000 → $10 por pip
        return 10.0
    elif kind == "INDEX":
        # Índices: 1 ponto = $1 por contrato (geralmente)
        return 1.0 * cs
    elif kind == "CRYPTO":
        # Crypto: 1 unidade = $1 do movimento
        return 1.0 * cs
    return 1.0
...
def commission_for(symbol, lot):
    """
    Comissão Round-Tick Tickmill Raw: $3 por lado por lote padrão.
    Para 0.01 lote = $0.03 por lado = $0.06 RT.
    """
...
def calc_lot_from_margin(symbol, entry, leverage, margin_usd):
    """
    Dado um valor de margem em USD, calcula o lote máximo possível.
    Lote = Margem × Leverage / (ContractSize × Price) [ajustado por tipo]
    """
    profile = symbol_profile(symbol)
    cs = float(profile["contract_size"])
    kind = profile["kind"]
    ...
    if kind == "FX":
        if quote == "USD":
            # EURUSD: Margem = lot × 100000 × price / leverage
            lot = margin_usd * leverage / (cs * entry)
        elif base == "USD":
            # USDJPY: Margem = lot × 100000 / leverage
...
def calc_lot_from_risk(symbol, entry, sl_price, balance, risk_pct):
    """
    Dado um risco % do balance, calcula o lote máximo permitido pelo risco.
    """
    ...
    # Valor perdido por lote padrão se SL for atingido
    profile = symbol_profile(symbol)
    cs = float(profile["contract_size"])
    kind = profile["kind"]
    quote = profile["quote"]
    ...
    if kind == "FX":
        if profile["quote"] == "USD":
            ...
            # USDJPY: sl_distance em JPY, converter
            quote_to_usd = currency_to_usd(quote)
        ...
        # CFDs: sl_distance em USD (ou moeda do ativo)
        loss_per_lot = sl_distance * cs
...
def calc_trade_plan(symbol, entry, leverage, balance, risk_pct, margin_usd):
    """
    Calcula o trade completo baseado no valor em USD que o usuário quer investir (margem).

    Retorna:
      - lot: lote calculado (limitado pela margem e pelo risco)
      - sl_price, tp_price: preços de SL e TP
      - sl_pct, tp_pct: porcentagens de SL e TP
      - margin_required: margem real necessária
      - risk_money: dinheiro em risco se SL for atingido
      - commission: comissão RT
      - potential_profit: lucro potencial se TP for atingido
    """
    entry = float(entry)
    ...
    # 1. Calcular SL/TP % automático baseado na alavancagem
    sl_pct, tp_pct = get_sl_tp_pct(leverage)

    # 2. Calcular preços de SL e TP (simétricos em %)
    # Nota: na prática o bot detecta direção, mas aqui calculamos ambos
    ...
    # 3. Calcular lote baseado na margem desejada
    lot_by_margin = calc_lot_from_margin(symbol, entry, leverage, margin_usd)

    # 4. Calcular lote baseado no risco máximo (usando SL de compra como referência)
    ...
    # 5. Lote final = menor entre margem e risco (nunca ultrapassa nenhum dos dois)
    final_lot = normalize_lot(min(lot_by_margin, lot_by_risk))

    if final_lot < Config.MIN_LOT:
        # Calcular margem mínima necessária para 0.01 lote
...
# ═══════════════════════════════════════════════════════════════
# PERSISTÊNCIA, NOTÍCIAS, ANÁLISE TÉCNICA (PRESERVADO)
# ═══════════════════════════════════════════════════════════════
def save_state(bot):
    data = {
...
class TradingBot:
    ...
    def confirm_pending(self, pending_id, amount=None):
        if amount is None:
            # Default: 10% do saldo ou $100, o que for maior
    ...
    def scan(self):
        ...
        for s in universe:
            ...
            # Usar SL/TP % automático baseado na alavancagem
...
def check_correlation(bot, symbol):
    ...
    if symbol in correlations:
        for related in correlations[symbol]:
            if related in active_symbols: return True
    return False
...
<title>Tickmill Sniper Bot v9.0</title>
<style>
:root{
...
.psym{font-size:20px;font-weight:700;font-family:var(--mono);color:var(--gold)}
.pname{font-size:11px;color:var(--muted2);margin-top:2px}
...
<div id="hdr">
  <div class="hdr-l">
    ...
    <div><div class="t1">Tickmill Sniper</div><div class="t2">MT5 • PRO v9.0</div></div>
  </div>
  <div class="hdr-r">
  ...
  el.innerHTML=list.length?list.map(p=>{
    ...
    return`<div class="pcard" data-pid="${p.pending_id}">
      ...
      <div style="font-size:11px;color:var(--muted2);margin-bottom:8px;text-align:center">
        💡 Clique no valor que deseja investir (margem em USD)
      </div>
      <div class="amt-grid">
      ...
      <div class="amt-input-wrap" id="custom-wrap-${p.pending_id}" style="display:none">
        <input type="number" min="1" step="1" placeholder="Valor em USD (ex: 750)" class="amt-input" id="custom-inp-${p.pending_id}" onkeydown="if(event.key==='Enter')submitCustomAmt(${p.pending_id})">
...
function toggleCustomAmt(id,btn){
  ...
  if(show)document.getElementById('custom-inp-'+id).focus();
}
...
async function loadNews(){
  try{
    ...
    document.getElementById('fg-card-wrap').innerHTML=`<div class="fg-card" style="background:var(--bg2);border:1px solid var(--border);border-radius:var(--r);padding:16px;margin-bottom:14px;display:flex;align-items:center;justify-content:space-between">
...
def create_api(bot):
    ...
    def api_health(): return jsonify({"status": "ok", "version": "9.0 PRO", "broker": Config.BROKER_NAME, "platform": Config.BROKER_PLATFORM, "account_type": Config.ACCOUNT_TYPE})
    ...
    def api_status():
        ...
        # SL/TP % atual baseado na alavancagem
...
# ═══════════════════════════════════════════════════════════════
# LOOP DO BOT & MAIN (100% PRESERVADO + AJUSTES v9)
# ═══════════════════════════════════════════════════════════════
def bot_loop(bot):
    bot.build_menu()
    if bot._restore_msg:
                ...
                for u in r["result"]:
                    ...
                    if "callback_query" in u:
                        ...
                        if cb.startswith("set_tf"): bot.set_timeframe(cb.replace("set_tf_", ""))
                            ...
                            try:
                                _, amt, pid = cb.split("_")
                                bot.execute_pending_with_amount(int(pid), float(amt), source="telegram")
