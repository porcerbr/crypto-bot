...
def get_dynamic_max_trades(balance=None):
    """Retorna máximo de trades ativos. No modo signal-only, não depende de capital."""
...
def get_allowed_symbols(balance=None):
    """Retorna a lista de símbolos monitorados, sem tier de capital."""
    selected = [s for s in get_selected_symbols() if s in Config.FXGOLD_ASSETS]
    return selected or list(Config.FXGOLD_ASSETS.keys())
...
def is_symbol_allowed(symbol, balance=None):
    """Verifica se o símbolo está dentro da lista monitorada."""
