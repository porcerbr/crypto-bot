...
def _apply_cost(price: float, direction: str, symbol: str) -> float:
    """Aplica spread + slippage aproximados no preço de entrada."""
    spread = float(Config.SPREAD_PIPS.get(symbol, 0.0) or 0.0)
    slippage = float(Config.SLIPPAGE_PIPS.get(symbol, 0.0) or 0.0)
    if symbol == "XAUUSD":
        pip_size = 0.1
        precision = 2
    elif is_jpy_pair(symbol):
        pip_size = 0.01
        precision = 3
    else:
        pip_size = 0.0001
        precision = 5
    impact = (spread * 0.5 + slippage) * pip_size
    if direction == "BUY":
        return round(price + impact, precision)
    return round(price - impact, precision)
...
def _signal(
    ...
    p = res["price"]
    d21 = res["dist_e21"]
    adx_min = float(adx_min if adx_min is not None else (14 if tf == "W1" else 16))
    pull_range = pull_range or ((-1.8, 3.0) if tf == "W1" else (-1.5, 2.5))
    min_confluence = max(1, min(7, int(min_confluence or 5)))
...
def run_backtest(
    ...
    for i in range(wb, len(bars)):
        ...
        rr = float(atr_tp_mult) / max(0.1, float(atr_sl_mult))
        if min_rr is not None and rr < float(min_rr):
            continue
